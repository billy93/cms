...
class BoqService
{
    ...
    public function createBoq(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            foreach ($items as $itemData) {
                ...
                $product = Product::with(['activePriceVersion'])->find($itemData['product_id']);
                if (!$product) {
                    throw new Exception("Product with ID {$itemData['product_id']} not found.");
                }

                // ⭐ Get active version
                $activePriceVersion = $product->activePriceVersion;
                if (!$activePriceVersion) {
                    throw new Exception("Product {$product->name} has no active version.");
                }

                // Snapshot data from active version
                $itemData['product_price_version_id'] = $activePriceVersion->id;
                $itemData['selling_price'] = (float) str_replace(['.', ','], ['', '.'], $itemData['selling_price']);

                $qty = $itemData['qty'];
                $itemData['qty_unit'] = $product->unit;
                $freq = $itemData['freq'];
                $multiplier = $qty * $freq * $itemData['selling_price'];

                $itemData['total_price'] = round($multiplier, 2);
                $boq->items()->create($itemData);
                $totalAmountItems += $multiplier;
            }

            $boq->update([
                'total_amount_items' => round($totalAmountItems, 2),
            ]);
    ...
    {
        $boq = Boq::with(['proposal', 'items.product.activePriceVersion'])->find($id);
        if (!$boq) {
    ...
    public function updateBoq($id, array $data)
    {
        return DB::transaction(function () use ($id, $data) {
            ...
            foreach ($items as $itemData) {
                ...
                // Snapshot data from active version
                $itemData['product_version_id'] = $activePriceVersion->id;
                $itemData['selling_price'] = (float) str_replace(['.', ','], ['', '.'], $itemData['selling_price']);
